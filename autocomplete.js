...
_b.AutoSuggest.prototype.setSuggestions = function (req, input)
{
	...
	if (this.oP.json)
	{
		...
		{
			this.aSug.push(  { 'id':jsondata.results[i].id, 'value':jsondata.results[i].value, 'regn':jsondata.results[i].regn, 'locstate':jsondata.results[i].locstate }  );
		}
	}
	else
	{
		...
		for (var i=0;i<results.length;i++)
		{
			if (results[i].hasChildNodes())
				this.aSug.push(  { 'id':results[i].getAttribute('id'), 'value':results[i].childNodes[0].nodeValue, 'regn':results[i].getAttribute('regn'), 'locstate':results[i].getAttribute('locstate') }  );
		}	
	}	
...
_b.AutoSuggest.prototype.createList = function(arr)
{
	...
	for (var i=0;i<arr.length;i++)
	{
		...
		//	var br = _b.DOM.cE("br", {});
		//	span.appendChild(br);
		//	var small = _b.DOM.cE("small", {}, arr[i].locstate);
	...
	// set mouseover functions for div when mouse pointer leaves div, set a timeout to remove the list after an interval when mouse enters div, kill the timeout so the list won't be removed
